...
class BasicCNNFastKAN(nn.Module):
    def __init__(self, hidden_dims=None, num_classes=10, freeze_backbone=False):
...
model = BasicCNNFastKAN(
    hidden_dims=[128, 64],  # Hidden layers in FastKAN
    num_classes=10,         # For CIFAR-10 or similar
    freeze_backbone=False   # Allow CNN training
).to(device)
...
for h, w in test_sizes:
    try:
        with torch.no_grad():
            test_input = torch.randn(2, 3, h, w).to(device)  # Batch size 2
            output = model(test_input)
            print(f"Input {h}x{w}: {test_input.shape} -> Output: {output.shape}")
