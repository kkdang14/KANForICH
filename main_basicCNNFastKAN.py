...
from FastKAN_model.BasicCNNFastKAN import BasicCNNFastKAN  # Assuming ConvNeXtFastKAN is defined in fastKCN.py
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir, model_name="basiccnn_fastkan"):
...
def train(model, train_loader, criterion, optimizer, device, epoch, config):
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):
        ...
        print(f"\rEpoch {epoch+1}/{config['num_epochs']} - Batch {batch_idx+1}/{num_batches} - Running Loss: {running_loss/total:.4f}, Running Acc: {100.*correct/total:.2f}% [{batch_progress:.1f}%]", end='', flush=True)

    epoch_loss = running_loss / len(train_loader.dataset)
    epoch_acc = 100. * correct / total
    print(f"Epoch {epoch+1}/{config['num_epochs']} - Train Loss: {epoch_loss:.4f}, Train Acc: {epoch_acc:.2f}% [{(epoch+1)/{config['num_epochs']}*100:.1f}%]", end='\r', flush=True)
...
def validate(model, val_loader, criterion, device, epoch, config):
    ...
    with torch.no_grad():  # Disable gradient tracking for validation
        for batch_idx, (inputs, labels) in enumerate(val_loader):
            ...
            batch_progress = (batch_idx + 1) / num_batches * 100
            print(f"\rEpoch {epoch+1}/{config['num_epochs']} - Batch {batch_idx+1}/{num_batches} - Val Running Loss: {running_loss/total:.4f}, Val Running Acc: {100.*correct/total:.2f}% [{batch_progress:.1f}%]", end='', flush=True)

    # Calculate epoch metrics
    epoch_loss = running_loss / len(val_loader.dataset)
    epoch_acc = 100. * correct / total
    print(f"Epoch {epoch+1}/{config['num_epochs']} - Val Loss: {epoch_loss:.4f}, Val Acc: {epoch_acc:.2f}% [{(epoch+1)/{config['num_epochs']}*100:.1f}%]", end='\r', flush=True)
...
def main():
    ...
    model = BasicCNNFastKAN().to(device)
    ...
    checkpoint_manager = CheckpointManager(config['checkpoint_dir'], "basiccnn_fastkan")
    ...
    plot_path = os.path.join(config['results_dir'], 'BasicCNNFastKAN_training_history.png')
