...
class ResNetMLP(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Use ResNet18 backbone from torchvision
        resnet = resnet18(pretrained=False)
        self.features = nn.Sequential(*list(resnet.children())[:-2])  # Remove original FC layer and avgpool
        
        # Global Average Pooling
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        
        # MLP layers for classification
        self.fc1 = nn.Linear(512, 128)  # ResNet18 outputs 512 features
...
model = ResNetMLP(num_classes=10).to(device)
