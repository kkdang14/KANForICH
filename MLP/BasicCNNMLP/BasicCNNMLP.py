...
class BasicCNNMLP(nn.Module):
    def __init__(self, num_classes=20):
        super(BasicCNNMLP, self).__init__()
        ...
        # MLP layers for classification (replacing KAN)
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        x = nn.ReLU()(x)  # Adding ReLU activation between MLP layers
        x = self.dropout(x)
        x = self.fc2(x)
...
model = BasicCNNMLP(num_classes=10).to(device)
